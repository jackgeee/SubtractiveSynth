...
double EnvelopeGenerator::nextSample() {
    ...
    return currentLevel;        // current level init = 0.0001
}
...
double EnvelopeGenerator::testCalculateMultiplier(double startLevel,
    double endLevel,
    unsigned long long lengthInSamples) {
    multiplier = 1.0 + (log(endLevel) - log(startLevel)) / (lengthInSamples);
    return multiplier; 
...
double EnvelopeGenerator::testSetSampleRate(double newSampleRate) {
    sampleRate = newSampleRate;
    return sampleRate; 
}
...
double EnvelopeGenerator::testSetStageValue(EnvelopeStage stage,
    double value) {
    stageValue[stage] = value;
    if (stage == currentStage) {
        // Re-calculate the multiplier and nextStageSampleIndex
